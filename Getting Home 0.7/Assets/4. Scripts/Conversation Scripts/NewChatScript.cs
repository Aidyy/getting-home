...
public class NewChatScript : MonoBehaviour {
	...
	void ChatActive () {
		...
		if (chatEnabled) {
			...
			if (charActive == CharacterChatActive.MotherBear) {
				...
				if (questReliantNpc.questStarted && bearCubTalkedto && !beaverTrigger) {
					...
					if (bearCubTalkedto && !objectiveCompleted) {
						textLines = (questRequest.text.Split ('\n'));
					...
					if (questStarted) {
						...
						if (!currentNpcScript.doesCharHaveItemReq && questStarted && !objectiveCompleted) {
							textLines = (questReminder.text.Split ('\n'));
					...
					if (Input.GetKeyDown (KeyCode.Return)) {
						...
						if (currentLine == endAtLine) {
							...
							if (questStarted && currentNpcScript.doesCharHaveItemReq) {
								...
								beaverTrigger = true;
								playerScript.currentHeldItem = null;
								currentNpcScript.doesCharHaveItemReq = false;
							}
						}
					}
				...
				else if (beaverTrigger && objectiveCompleted)
				{
					textLines = (idleDialogue.text.Split('\n'));	
					BeaverRandomRoutine();
				}
				...
				if (questReliantNpc.questStarted != true || questReliantNpc.questStarted == true && beaverTalkedto != true || objectiveCompleted == true) {
					textLines = (idleDialogue.text.Split ('\n'));
	...
	{
	if (other.tag == "Player") {
			playerConsole = other.GetComponent<PlayerScript> ();
			chatTrigger = true;		
		}
	}
	...
	void ChatDisable()
	{
		chatEnabled = false;
		theText.enabled = false;
		charIdentifier.enabled = false;
	}
	void BeaverRandomRoutine()
	{
		if (Input.GetKeyDown (KeyCode.Return)) {
